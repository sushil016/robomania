  ...
  try {
    ...
    if (finalTeamId && competitionsArray.length > 0) {
      ...
      for (const comp of competitionsArray) {
        const { data: existingReg } = await supabaseAdmin.from('competition_registrations').select('id').eq('team_id', finalTeamId).eq('competition_type', comp.competition.toUpperCase()).single()

        if (existingReg) {
          await supabaseAdmin.from('competition_registrations').update({ payment_id: order.id, payment_status: 'PENDING', amount: comp.amount }).eq('id', existingReg.id)
          console.log(`✅ Updated registration for \${comp.competition}`)
        } else {
          await supabaseAdmin.from('competition_registrations').insert({ team_id: finalTeamId, competition_type: comp.competition.toUpperCase(), bot_id: comp.botId || null, amount: comp.amount, payment_id: order.id, payment_status: 'PENDING', registration_status: 'PENDING' })
